...
const Dashboard = () => {
    ...
    useEffect(() => {
        const fetchCars = async () => {
            try {
                const response = await fetch("http://localhost:8005/api/getcar");
    ...
    const handleDelete = async (carId) => {
        try {
            const response = await fetch(`http://localhost:8005/api/car/${carId}`, {
                method: "DELETE",
            });
                        ...
                        <Row className="align-items-center">
                            <Col xs={12} md={4}>
                                ...
                                <div className="car-image">
                                    {car.image && (
                                        <img
                                            src={`http://localhost:8005/api/car/${car.id}/image`} // Assuming your backend serves images at this endpoint
                                            alt={car.name}
                                            className="img-fluid"
                                        />
                                    )}
